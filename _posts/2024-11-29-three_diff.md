...
where $\hat{\mu}_t, \hat{\sigma}_t$ are simple functions (for example, in DDPM we have $\hat{\mu}_t(x)=\sqrt{1-\beta_t}x,\hat{\sigma}_t(x)=\sqrt{{\beta}_t}$, whereas $\beta_t$ are some hyperparameters, called the "diffusion schedule"). 
